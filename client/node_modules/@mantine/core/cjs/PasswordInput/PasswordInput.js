...
const PasswordInput = React.forwardRef((props, ref) => {
  ...
  const { classes, cx } = PasswordInput_styles['default']({ size, rightSectionWidth }, { name: "PasswordInput", classNames, styles: styles$1, unstyled });
  const uuid = hooks.useId(id);
  const { systemStyles, rest } = styles.extractSystemStyles(others);
  const [reveal, { toggle }] = hooks.useDisclosure(false);
